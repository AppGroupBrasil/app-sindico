...
import {
  ...
  Clock, CheckCircle2, FileText, Wrench, Building2, Search, Filter, ChevronDown, ChevronUp, BookmarkPlus, ImagePlus
} from 'lucide-react';
...
const formVazio = (): Omit<Vencimento, 'id' | 'criadoEm'> => ({
  titulo: '', tipo: 'contrato', descricao: '', condominio: '',
...
const VencimentosPage: React.FC = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setEmailsSalvos(emails as string[]);
  ...
  /* ── modal open/close ── */
  const abrirNovo = () => { if (!tentarAcao()) return; setForm(formVazio()); setEditandoId(null); setEmailInput(''); setModalAberto(true); };
  const abrirEditar = (v: Vencimento) => {
    if (!tentarAcao()) return;
    setForm({ titulo: v.titulo, tipo: v.tipo, descricao: v.descricao, condominio: v.condominio, dataVencimento: v.dataVencimento, dataUltimaManutencao: v.dataUltimaManutencao || '', dataProximaManutencao: v.dataProximaManutencao || '', emails: [...v.emails], avisos: v.avisos.map(a => ({ ...a })), qtdNotificacoes: v.qtdNotificacoes, imagens: [...(v.imagens || [])] });
        ...
        <div className={styles.modalForm}>
          ...
          <div className={styles.formGroup}>
            ...
            <select className={styles.formSelect} value={form.condominio} onChange={e => setForm(p => ({ ...p, condominio: e.target.value }))}>
              ...
              {condominiosList.map(c => <option key={c.id} value={c.nome}>{c.nome}</option>)}
            </select>
          </div>
          {/* Galeria de imagens */}
          <div className={styles.formGroup}>
