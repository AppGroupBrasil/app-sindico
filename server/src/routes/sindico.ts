...
router.get('/resumo', async (req: AuthRequest, res: Response) => {
  ...
  const [condsCount, osStats, solStats, moradoresCount, custoMes, osRecentes, comunicadosCount] = await Promise.all([
    ...
    queryOne<any>(
      `SELECT
         COUNT(*) FILTER (WHERE status IN ('aberta','em_analise')) as pendentes,
         COUNT(*) as total
  ...
  res.json({
    ...
    solicitacoesPendentes: parseInt(solStats?.pendentes || '0'),
    solicitacoesTotal: parseInt(solStats?.total || '0'),
