...
router.get('/summary', async (req: AuthRequest, res: Response) => {
  ...
  const [
    ...
    query(`
      SELECT d::date as dia,
        COALESCE((SELECT COUNT(*)::int FROM reportes WHERE condominio_id IN (${ph}) AND criado_em::date = d::date), 0) as abertas,
        COALESCE((SELECT COUNT(*)::int FROM reportes WHERE condominio_id IN (${ph}) AND criado_em::date = d::date AND status = 'resolvido'), 0) +
    ...
    query(`SELECT protocolo, status, prioridade, criado_em FROM reportes WHERE condominio_id IN (${ph}) ORDER BY criado_em DESC LIMIT 10`, ids),
  ...
  for (const v of (atividadeVenc || [])) {
    ...
    atividades.push({
      ...
      tempo: formatarTempo(dv.getTime()), ts: dv.getTime(),
