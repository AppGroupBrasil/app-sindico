...
const DemoEntryPage: React.FC = () => {
  ...
  const { login, logout, usuario, loginDireto } = useAuth();
  ...
  useEffect(() => {
    const iniciarDemo = async () => {
      ...
      localStorage.setItem('manutencao_user', JSON.stringify(demoUser));
      localStorage.setItem('manutencao-ultimo-condo', demoProfile.condominioId);
