...
const EquipamentosPage: React.FC = () => {
  ...
  const [histEquip, setHistEquip] = useState<any | null>(null);
  ...
  useEffect(() => {
    Promise.all([
      ...
      setFornecedoresList(forns as any[]);
  ...
  const salvar = async () => {
    ...
    try {
      if (editando) {
        ...
        setEquipamentos(prev => prev.map(e => e.id === editando.id ? atualizado : e));
      } else {
          ...
          <div className={styles.historicoList}>
            ...
            {historico.map(h => (
              <div className={styles.historicoItem} key={h.id}>
                <h4>{h.tipo?.toUpperCase()} — {new Date(h.dataServico).toLocaleDateString('pt-BR')}</h4>
                <p>{h.descricao}</p>
                {h.tecnico && <p>Técnico: {h.tecnico}</p>}
                {h.fornecedorNome && <p>Fornecedor: {h.fornecedorNome}</p>}
                {h.custo > 0 && <p style={{ fontWeight: 600 }}>Custo: R$ {Number(h.custo).toFixed(2)}</p>}
              </div>
            ))}
          </div>
        </Modal>
