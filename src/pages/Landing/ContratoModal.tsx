...
const ContratoModal: React.FC<ContratoModalProps> = ({ plano, onClose }) => {
  ...
  const handlePrint = () => {
    ...
    win.document.write(`
      <html>
        <head>
          <title>Contrato - App Síndico</title>
          <style>
  ...
  const handleShare = async () => {
    const texto = `Contrato de Prestação de Serviços - App Síndico\n\nPlano: R$${plano.preco}/mês (até ${plano.usuarios} usuários)\n\nPara mais detalhes, acesse: ${window.location.origin}`;

    if (navigator.share) {
      try {
        await navigator.share({ title: 'Contrato - App Síndico', text: texto });
      } catch {
        /* user cancelled */
      }
    } else {
        ...
        <div className={styles.modalBody} ref={contratoRef}>
          <div className={styles.contratoTexto}>
            ...
            <p className={styles.clausula}>
              ...
              denominado <strong>App Síndico</strong>, "sistema de gestão condominial completo",
              disponibilizado pela CONTRATADA à CONTRATANTE por meio da internet (www.appsindico.com.br).
            </p>
            ...
            <p className={styles.clausula}>
              A CONTRATADA disponibilizará à CONTRATANTE acesso completo ao sistema App Síndico,
            ...
            <p className={styles.clausula}>
              O software App Síndico é de propriedade exclusiva da CONTRATADA.
