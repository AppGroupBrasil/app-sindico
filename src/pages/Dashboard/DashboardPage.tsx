...
const MasterDashboard: React.FC<{ usuario: any }> = ({ usuario }) => {
  ...
  const handleResetSenha = async (id: string, nome: string) => {
    if (!confirm(`Resetar senha de s${nome}s para 123456?`)) return;
    setActionLoading(id);
    try {
  ...
  const handleExcluir = async (id: string, nome: string) => {
    if (!confirm(`Excluir s${nome}s? Esta ação irá inativar o usuário.`)) return;
    setActionLoading(id);
    try {
  ...
  const exportReportCSV = () => {
    ...
    for (const u of (reportData.usuarios || [])) {
      csv += `Usuário,s${u.nome}s,s${u.email}s,${u.role},${formatDate(u.criadoEm)},s${u.adminNome || ''}s,s${u.condominioNome || ''}s\n`;
    }
    for (const c of (reportData.condominios || [])) {
      csv += `Condomínio,s${c.nome}s,s${c.adminEmail || ''}s,${c.statusPlano},${formatDate(c.criadoEm)},s${c.adminNome || ''}s,${c.totalUsuarios} usu / ${c.totalMoradores} mor\n`;
    }
  ...
  return (
    <div id="dashboard-content">
      ...
      <PageHeader
        ...
        onCompartilhar={() => compartilharConteudo('Dashboard Master', 'Resumo da plataforma App Síndico')}
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(135deg, #1E3A5F 0%, #2A5A8F 60%, #D4AF37 100%)',
          borderRadius: 16,
          padding: '20px 24px',
          marginBottom: 24,
          cursor: 'pointer',
        ...
        onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 30px rgba(30,58,95,0.3)'; }}
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ color: '#fff', fontWeight: 700, fontSize: 16 }}>Revista do Condomínio</div>
          <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginTop: 2 }}>Edição de Março 2026 — Clique para ler</div>
        </div>
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.6)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 5l7 7-7 7" /></svg>
      </div>
        ...
        {filtered.length === 0 && (
          <Card padding="md">
            <div style={{ textAlign: 'center', padding: 30, color: 'var(--cor-texto-secundario)' }}>
              Nenhum resultado encontrado para s{busca}s
            </div>
          </Card>
        )}
      </>
  ...
  return (
    <div id="dashboard-content">
      ...
      <PageHeader
        ...
        onCompartilhar={() => compartilharConteudo('Dashboard', 'Resumo do sistema App Síndico')}
        onImprimir={() => imprimirElemento('dashboard-content')}
